...
import org.leave.flink.practice.orderpay.bean.ReceiptEvent;
import org.leave.flink.practice.orderpay.function.TxPayMatchByJoin;
 ...
 */
@Slf4j
public class TxMatchByJoinTask {
    public static void main(String[] args) throws Exception {
        ...
        DataStream<Tuple2<OrderEvent, ReceiptEvent>> processedStream = orderEventStream.intervalJoin(receiptEventStream)
                .between(Time.seconds(-5), Time.seconds(5))
                .process(new TxPayMatchByJoin());

        processedStream.print("matched");

        env.execute("TxMatchByJoinTask");
